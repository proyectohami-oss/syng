  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(160deg,#0f2a4a,#185FA5,#534AB7)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'space-between', padding:'48px 32px', fontFamily:'-apple-system,BlinkMacSystemFont,sans-serif' }}>
      ...
      <div style={{ textAlign:'center', flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'24px', width:'100%', maxWidth:'360px' }}>
        ...
        {/* Si ya tiene sesión — mostrar su nombre */}
        ...
        <button
          ...
          disabled={entrando}
          style={{ width:'100%', padding:'18px', background:'white', color:'#185FA5', border:'none', borderRadius:'16px', fontSize:'17px', fontWeight:'700', cursor:'pointer', marginTop:'8px', opacity: entrando ? 0.7 : 1, WebkitTapHighlightColor:'transparent' }}>
          {entrando ? 'Entrando...' : 'Entrar al grupo'}
        </button>

        {/* Si no tiene sesión — opción de ir a login */}
        {(!userActual || userActual.isAnonymous) && (
          <div style={{ textAlign:'center' }}>
            ...
            <button
              onClick={onIrLogin}
              style={{ background:'rgba(255,255,255,0.12)', border:'1px solid rgba(255,255,255,0.25)', color:'white', borderRadius:'12px', padding:'10px 28px', fontSize:'14px', fontWeight:'500', cursor:'pointer', WebkitTapHighlightColor:'transparent' }}>
              Iniciar sesión
            </button>
          </div>
        )}
