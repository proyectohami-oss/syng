...
export default function Pizarron({ onVolver }) {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#f5f5f7',fontFamily:'-apple-system,BlinkMacSystemFont,sans-serif'}}>
      ...
      {modalDia&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:100,touchAction:'none'}} onClick={e=>{if(e.target===e.currentTarget)cerrarModal()}}>
          <div style={{background:'white',borderRadius:'24px 24px 0 0',width:'100%',maxWidth:'600px',maxHeight:'88vh',overflowY:'auto',padding:'24px',overscrollBehavior:'contain',touchAction:'pan-y'}} onClick={e=>e.stopPropagation()}>
            ...
            {(anotaciones[getKey(anio,mes,modalDia)]||[]).map((a,i)=>(
              <div key={a.id}>
                ...
                <div data-modalidx={i} onMouseDown={e=>onMouseDownModal(e,i)} onTouchStart={e=>onTouchStartModal(e,i)} onTouchMove={onTouchMoveModal} onTouchEnd={onTouchEndModal} style={{padding:'8px 0',borderBottom:'1px solid #f5f5f7',userSelect:'none',opacity:draggingModalIdx===i?0.4:1,borderRadius:'8px',touchAction:isDraggingModal.current?'none':'pan-y'}}>
                  {editando===a.id?(
                    <div>
