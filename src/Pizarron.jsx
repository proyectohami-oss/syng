...
export default function Pizarron({ onVolver }) {
  ...
  const abrirModal = (dia) => { setModalDia(dia); setTextoNuevo(''); setMostrarRepetir(false); setFechasRepetir([{dia,mes,anio}]); setMesRepetir(mes); setAnioRepetir(anio); setEditando(null); setEditFecha(null); setMostrarCalEditFecha(false) }
  const cerrarModal = () => { setModalDia(null); setEditando(null); setConfirmEliminar(null); setMostrarRepetir(false); setFechasRepetir([]); setEditFecha(null); setMostrarCalEditFecha(false); setDragOverModal(null); setDraggingModalIdx(null); isDraggingModal.current=false }
  ...
  const iniciarEdicion = (a, idx) => { setEditando(a.id); editandoIdxRef.current=idx; setTextoEditar(a.texto); setEditFecha({dia:a.dia,mes:a.mes,anio:a.anio}); setMesCalEdit(a.mes); setAnioCalEdit(a.anio); setMostrarCalEditFecha(false) }
  const repetirEnFechas = (idTarea, diaOrigen) => {
    const tareaOrigen = tareas.find(t => t.id === idTarea)
    if (!tareaOrigen) return
    const nuevasTareas = [...tareas]
    fechasRepetir.forEach(k => {
      const [anio, mes, dia] = k.split('-').map(Number)
      nuevasTareas.push({...tareaOrigen, id: Date.now()+Math.random(), dia, mes, anio, realizada: false})
    })
                  ...
                  {editando===a.id?(
                    <div>
                      ...
                      <div style={{marginLeft:'32px'}}>
                        <div style={{display:'flex',gap:'8px',marginBottom:'8px'}}>
                          <button onClick={()=>setEditModo('fecha')} style={{background:editModo==='fecha'?'#534AB7':'#EEF2FF',border:'none',borderRadius:'8px',padding:'6px 12px',fontSize:'13px',color:editModo==='fecha'?'white':'#534AB7',fontWeight:'500',...T}}>📅 Cambiar fecha</button>
                          <button onClick={()=>setEditModo('repetir')} style={{background:editModo==='repetir'?'#534AB7':'#EEF2FF',border:'none',borderRadius:'8px',padding:'6px 12px',fontSize:'13px',color:editModo==='repetir'?'white':'#534AB7',fontWeight:'500',...T}}>🔁 Repetir en más fechas</button>
                        </div>
                        {editModo==='fecha'&&(<div style={{background:'#f5f5f7',borderRadius:'12px',padding:'12px',marginTop:'4px'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px'}}><button onClick={()=>{if(mesCalEdit===0){setMesCalEdit(11);setAnioCalEdit(anioCalEdit-1)}else setMesCalEdit(mesCalEdit-1)}} style={{background:'white',border:'1px solid #e5e5e5',borderRadius:'8px',padding:'3px 10px',...T}}>‹</button><div style={{fontSize:'13px',fontWeight:'600'}}>{MESES[mesCalEdit]} {anioCalEdit}</div><button onClick={()=>{if(mesCalEdit===11){setMesCalEdit(0);setAnioCalEdit(anioCalEdit+1)}else setMesCalEdit(mesCalEdit+1)}} style={{background:'white',border:'1px solid #e5e5e5',borderRadius:'8px',padding:'3px 10px',...T}}>›</button></div><div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'3px',marginBottom:'4px'}}>{DIAS_SEMANA.map(d=><div key={d} style={{textAlign:'center',fontSize:'10px',color:'#aaa'}}>{d}</div>)}</div><div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'3px'}}>{celdasCalEdit.map((d,i)=><button key={i} onClick={()=>d&&seleccionarFechaEdicion(d)} style={{height:'32px',display:'flex',alignItems:'center',justifyContent:'center',borderRadius:'6px',fontSize:'12px',background:d&&editFecha?.dia===d&&editFecha?.mes===mesCalEdit&&editFecha?.anio===anioCalEdit?'#534AB7':d?'white':'transparent',color:d&&editFecha?.dia===d&&editFecha?.mes===mesCalEdit&&editFecha?.anio===anioCalEdit?'white':'#2C2C2A',border:d?'1px solid #e5e5e5':'none',...T}}>{d||''}</button>)}</div></div>)}
                        {editModo==='repetir'&&(<div style={{background:'#f5f5f7',borderRadius:'12px',padding:'12px',marginTop:'4px'}}><div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'10px'}}><button onClick={()=>{if(mesRepetir===0){setMesRepetir(11);setAnioRepetir(anioRepetir-1)}else setMesRepetir(mesRepetir-1)}} style={{background:'white',border:'1px solid #e5e5e5',borderRadius:'8px',padding:'3px 10px',...T}}>‹</button><div style={{fontSize:'13px',fontWeight:'600'}}>{MESES[mesRepetir]} {anioRepetir}</div><button onClick={()=>{if(mesRepetir===11){setMesRepetir(0);setAnioRepetir(anioRepetir+1)}else setMesRepetir(mesRepetir+1)}} style={{background:'white',border:'1px solid #e5e5e5',borderRadius:'8px',padding:'3px 10px',...T}}>›</button></div><div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'3px',marginBottom:'4px'}}>{DIAS_SEMANA.map(d=><div key={d} style={{textAlign:'center',fontSize:'10px',color:'#aaa'}}>{d}</div>)}</div><div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:'3px'}}>{celdasRepetir.map((d,i)=>{const k=`${anioRepetir}-${mesRepetir}-${d}`;const sel=fechasRepetir.includes(k);return(<button key={i} onClick={()=>{if(!d)return;if(sel)setFechasRepetir(fechasRepetir.filter(f=>f!==k));else setFechasRepetir([...fechasRepetir,k])}} style={{height:'32px',display:'flex',alignItems:'center',justifyContent:'center',borderRadius:'6px',fontSize:'12px',background:sel?'#534AB7':d?'white':'transparent',color:sel?'white':'#2C2C2A',border:d?'1px solid #e5e5e5':'none',...T}}>{d||''}</button>);})}</div>{fechasRepetir.length>0&&<button onClick={()=>repetirEnFechas(a.id,modalDia)} style={{marginTop:'10px',width:'100%',background:'#534AB7',border:'none',borderRadius:'8px',padding:'8px',color:'white',fontSize:'14px',fontWeight:'600',...T}}>Agregar en {fechasRepetir.length} fecha{fechasRepetir.length>1?'s':''}</button>}</div>)}
                      </div>
                    </div>
                  ):(
