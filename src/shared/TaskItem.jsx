    ...
    <div style={{ ...row, opacity: isPending ? 0.65 : 1 }} className="task-item">
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {(task.description || hasDueDate || showGroup || isPending) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2 }}>
            ...
            {showGroup && groupName && (
              <span style={{ fontSize: 11, padding: '1px 6px', borderRadius: 4, background: '#eff6ff', color: '#3b82f6', flexShrink: 0 }}>
                {groupName}
              </span>
            )}
            {isPending && (
      ...
      {!disabled && (
        <div className="task-actions" style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
          ...
          <button onClick={(e) => { e.stopPropagation(); onDelete(task) }} aria-label="Eliminar tarea" style={{ ...actionBtn, color: '#ef4444' }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const row = { display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 16px', borderBottom: '1px solid #f3f4f6', transition: 'background 0.1s' }
const toggle = { flexShrink: 0, width: 20, height: 20, borderRadius: '50%', border: '2px solid', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', padding: 0, transition: 'background 0.15s, border-color 0.15s' }
const actionBtn = { padding: 4, borderRadius: 4, border: 'none', background: 'transparent', cursor: 'pointer', color: '#9ca3af', display: 'flex', alignItems: 'center', justifyContent: 'center' }
