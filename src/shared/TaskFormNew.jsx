 ...
 *
 * RENDIMIENTO: onClose() se llama ANTES de que Firestore confirme.
 * El optimistic dispatch en useTasks ya actualizó el store.
 * El usuario ve el cambio instantáneamente y el formulario cierra de inmediato.
 * Firestore escribe en segundo plano.
 * Si hay error → el rollback en useTasks revierte el estado.
 */
...
export function TaskFormNew({ task, defaultDate, onClose }) {
  ...
  function handleSave() {
    ...
    // CLAVE: cerrar el modal INMEDIATAMENTE
    // El optimistic dispatch ya ocurre dentro de createTask/updateTask
    // antes de que Firestore responda.
    onClose()

    // Escribir a Firestore en segundo plano (sin await en el handler)
    if (isEdit) {
      updateTask(task, {
        title:   title.trim(),
        groupId: gId,
        ...
        console.error('[TaskFormNew] updateTask error:', err)
        setError(err.message)
      })

      if (repeatDays.size > 0) {
        const dias = Array.from(repeatDays).sort().filter(d => d !== dateStr)
        dias.forEach(day => {
          createTask({
            title:   title.trim(),
            type,
            groupId: gId,
            dueDate: Timestamp.fromDate(new Date(day + 'T23:59:59')),
          }).catch(console.error)
        })
      }
      ...
      createTask({
        ...
        console.error('[TaskFormNew] createTask error:', err)
      })
    }
  }
