...
// Selector de fecha propio (sin input nativo)
function SelectorFecha({ value, onChange }) {
  ...
  return (
    <div style={{ background: 'white', border: '1.5px solid #e5e5e5', borderRadius: '14px', padding: '12px', userSelect: 'none' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
        <button onClick={mesAnterior} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#185FA5', padding: '4px 8px' }}>‹</button>
        <span style={{ fontWeight: '600', fontSize: '14px', color: '#2C2C2A' }}>{MESES[mes]} {anio}</span>
        <button onClick={mesSiguiente} style={{ background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#185FA5', padding: '4px 8px' }}>›</button>
      </div>
        ...
        {celdas.map((dia, i) => {
          ...
          return (
            <button key={i} onClick={() => seleccionar(dia)}
              style={{
                width: '100%', aspectRatio: '1', border: 'none', borderRadius: '50%', cursor: 'pointer', fontSize: '13px', fontWeight: seleccionado || esHoy ? '700' : '400',
                background: seleccionado ? '#185FA5' : esHoy ? '#E6F1FB' : 'transparent',
                color: seleccionado ? 'white' : esHoy ? '#185FA5' : '#2C2C2A',
              }}>
              {dia}
            </button>
          )
...
export default function ListaTareas({ onVolver, userId }) {
  ...
  return (
    <div style={{ height: '100dvh', background: '#F5F5F7', fontFamily: '-apple-system,BlinkMacSystemFont,sans-serif', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      {/* Barra navegación días — estable, sin saltos */}
      <div style={{ background: '#0C447C', display: 'flex', alignItems: 'center', padding: '8px 12px', gap: '6px', flexShrink: 0 }}>
        ...
        {/* Botón Hoy — siempre visible pero atenuado si ya es hoy */}
        <button onClick={irHoy}
          style={{ width: '38px', height: '38px', borderRadius: '10px', background: diaActivo === hoyStr ? 'rgba(255,255,255,0.08)' : 'white', border: 'none', color: diaActivo === hoyStr ? 'rgba(255,255,255,0.3)' : '#0C447C', fontSize: '10px', fontWeight: '700', cursor: 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', lineHeight: 1.1, textAlign: 'center' }}>
          HOY
        </button>
      </div>
        ...
        {pendientes.map((t, i) => (
          <div key={t.id} style={{ background: 'white', borderBottom: '0.5px solid #EBEBEB' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '14px 16px' }}>
              ...
              <button onClick={() => setMenuTarea(menuTarea?.id === t.id ? null : t)}
            ...
            {menuTarea?.id === t.id && (
              <div style={{ display: 'flex', borderTop: '0.5px solid #f0f0f0' }}>
                <button onClick={() => abrirEditar(t)} style={{ flex: 1, padding: '13px', border: 'none', background: '#F0F6FF', color: '#185FA5', fontSize: '15px', fontWeight: '600', cursor: 'pointer', borderRight: '0.5px solid #dde8f5', borderRadius: '0 0 0 0' }}>✎ Editar</button>
          ...
          <>
            ...
            {completadas.map(t => (
              <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', background: '#FAFAFA', borderBottom: '0.5px solid #EBEBEB' }}>
                ...
                <button onClick={() => pedirEliminar(t)} style={{ background: 'none', border: 'none', color: '#ccc', fontSize: '18px', cursor: 'pointer' }}>✕</button>
              </div>
            ))}
          </>
        )}
      ...
      {/* Modal agregar */}
      {modalAgregar && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', zIndex: 100, paddingTop: '20px', overflowY: 'auto' }}
          onClick={e => { if (e.target === e.currentTarget) setModalAgregar(false) }}>
          <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '500px', margin: '0 16px 20px', padding: '24px 20px' }}>
