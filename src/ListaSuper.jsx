...
export default function ListaSuper({onVolver,tema='oscuro',idioma='es',userId=null,userName='Tú',userEmail='',grupoInicial=null}){
  ...
  useEffect(()=>{
    ...
    const unsub=onSnapshot(collection(db,'users',userId,'misGrupos'),async snap=>{
      ...
      setGrupos(lista)
      setGrupoActivo(prev=>prev==='personal'?'personal':(lista.length>0?lista[0].id:'personal'))
      setCargando(false)
    })
    return()=>unsub()
  },[userId])
  ...
  function agregarProducto(dep,nombre){setCustomProds(prev=>({...prev,[dep]:[...(prev[dep]||[]),nombre]}));setModal(null)}
  function editarProducto(dep,viejo,nuevo){setCustomProds(prev=>({...prev,[dep]:(prev[dep]||[]).map(p=>p===viejo?nuevo:p)}));setModal(null)}
