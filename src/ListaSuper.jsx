...
export default function ListaSuper({onVolver,tema='oscuro',idioma='es'}){
  ...
  const [grupos,setGrupos]=useState([{id:'familia',nombre:'Familia',color:'#5DCAA5',miembros:['Tú','Mamá','Papá']}])
  ...
  useEffect(()=>{
    const h=(e)=>{const{producto,departamento}=e.detail;const dep=departamento||'Abarrotes';setSeleccionados(prev=>({...prev,[producto]:{qty:1,done:false,dep}}))}
  ...
  function toggleProd(p,dep){setSeleccionados(prev=>{const next={...prev};if(next[p])delete next[p];else next[p]={qty:1,done:false,dep};return next})}
  function cambiarQty(p,delta){setSeleccionados(prev=>({...prev,[p]:{...prev[p],qty:Math.max(1,(prev[p]?.qty||1)+delta)}}))}
  function toggleDone(p){setSeleccionados(prev=>({...prev,[p]:{...prev[p],done:!prev[p].done}}))}
  function toggleListSel(p){setListSelIds(prev=>prev.includes(p)?prev.filter(i=>i!==p):[...prev,p])}
  function eliminarSeleccion(){setSeleccionados(prev=>{const next={...prev};listSelIds.forEach(p=>delete next[p]);return next});setListSelIds([]);setListSelMode(false)}
  function borrarLista(){setSeleccionados({});setListSelIds([]);setListSelMode(false);setModal(null)}
  function borrarMarcados(){setSeleccionados(prev=>{const next={};Object.entries(prev).forEach(([k,v])=>{if(!v.done)next[k]=v});return next});setModal(null)}
  function agregarProducto(dep,nombre){setCustomProds(prev=>{const arr=[...(prev[dep]||[]),nombre];return{...prev,[dep]:arr}});setModal(null)}
  function editarProducto(dep,viejo,nuevo){setCustomProds(prev=>{const arr=(prev[dep]||[]).map(p=>p===viejo?nuevo:p);return{...prev,[dep]:arr}});setSeleccionados(prev=>{if(!prev[viejo])return prev;const next={...prev,[nuevo]:prev[viejo]};delete next[viejo];return next});setModal(null)}
  function eliminarProductoCat(dep,prod){setCustomProds(prev=>({...prev,[dep]:(prev[dep]||[]).filter(p=>p!==prod)}));setSeleccionados(prev=>{const next={...prev};delete next[prod];return next});setModal(null)}
  function crearGrupo(nombre){const color=GRUPO_COLORS[grupos.length%GRUPO_COLORS.length];setGrupos(prev=>[...prev,{id:generarId(),nombre,color,miembros:['Tú']}]);setGrupoActivo(grupos.length);setSeleccionados({});setModal(null)}
  function cambiarGrupo(i){setGrupoActivo(i);setSeleccionados({});setModal(null)}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:100,padding:'20px'}} onClick={e=>{if(e.target===e.currentTarget)setModal(null)}}>
          {modal==='grupos'&&(
            <div style={{background:th.modalBg,borderRadius:'16px',padding:'22px 20px',width:'100%',maxWidth:'340px'}}>
              ...
              {grupos.map((gr,i)=>(
                <div key={gr.id} onClick={()=>cambiarGrupo(i)} style={{display:'flex',alignItems:'center',gap:'10px',padding:'10px 0',borderBottom:`0.5px solid ${th.borde}`,cursor:'pointer'}}>
