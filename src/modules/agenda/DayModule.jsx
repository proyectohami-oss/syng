...
// Modal para editar múltiples tareas
    ...
    <div style={overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={sheet}>
        ...
        <p style={{ fontSize:13, color:'#9ca3af', margin:'0 0 16px' }}>
          Solo se aplican los campos que cambies.
        </p>
        ...
        <div style={{ background:'#f9fafb', borderRadius:10, overflow:'hidden', border:'1px solid #f3f4f6' }}>
          {[
            { id:'__sin_cambio__', label:'Sin cambio' },
            { id:'',               label:'Personal' },
            ...groups.map(g => ({ id: g.id, label: g.name })),
          ].map(op => (
            <div
              key={op.id}
              onClick={() => setGroupId(op.id)}
              style={{
                padding:'11px 16px', fontSize:14, cursor:'pointer',
                borderBottom:'1px solid #f3f4f6',
                background: groupId === op.id ? '#EDE9FE' : 'transparent',
                color:      groupId === op.id ? '#5B3DF6' : '#374151',
                fontWeight: groupId === op.id ? 600 : 400,
              }}
            >
              {op.label}
            </div>
        ...
        <div style={{ marginTop:20, display:'flex', flexDirection:'column', gap:10 }}>
          <button
            onClick={guardar}
            disabled={!hayCambio || loading}
            style={{
              width:'100%', padding:'13px', borderRadius:12, border:'none',
              fontSize:15, fontWeight:600, cursor: hayCambio ? 'pointer' : 'default',
              background: hayCambio ? '#5B3DF6' : '#e5e7eb',
              color:      hayCambio ? '#fff'    : '#9ca3af',
            }}
          >
            {loading ? 'Aplicando...' : 'Aplicar cambios'}
          </button>
          <button onClick={onClose} style={{ background:'none', border:'none', color:'#6b7280', fontSize:15, cursor:'pointer', padding:'8px' }}>
            Cancelar
          </button>
        </div>
      </div>
...
export function DayModule() {
  ...
  // Empieza vacío. Barra inferior aparece/desaparece según selectedIds.size.
  const [selectedIds,  setSelectedIds]  = useState(new Set())
  const [hiddenIds,    setHiddenIds]    = useState(new Set()) // ocultado localmente antes de Firestore
  ...
  const pendingOrdenados = (orden ?? pending).filter(t => !hiddenIds.has(t.id))
  const completedVisible = completed.filter(t => !hiddenIds.has(t.id))

  function onDragStart(id) { dragRef.current = id }
  function onDragOver(id)  { if (dragRef.current && dragRef.current !== id) setDragOver(id) }
  function onDrop(targetId) {
    const srcId = dragRef.current
    if (!srcId || srcId === targetId) { dragRef.current = null; setDragOver(null); return }
    const arr = [...pendingOrdenados]
    const fi  = arr.findIndex(t => t.id === srcId)
    const ti  = arr.findIndex(t => t.id === targetId)
    if (fi === -1 || ti === -1) return
    arr.splice(fi, 1); arr.splice(ti, 0, pendingOrdenados[fi])
    setOrden(arr); dragRef.current = null; setDragOver(null)
  }
  function onDragEnd() { dragRef.current = null; setDragOver(null) }
  ...
  async function eliminarSeleccionadas() {
    // Ocultar todas instantáneamente
  ...
  async function editarSeleccionadas({ fecha, groupId }) {
    ...
    for (const id of selectedIds) {
      ...
      const updates = {}
      if (fecha !== '') {
        updates.dueDate = Timestamp.fromDate(new Date(fecha + 'T23:59:59'))
      }
      if (groupId !== '__sin_cambio__') {
        updates.groupId = groupId || null
        updates.type    = groupId ? 'group' : 'personal'
      }
  ...
  return (
    <div style={screen}>
      ...
      {/* Header */}
      <div style={header}>
        <button onClick={() => navigate('/agenda')} style={btnVolver}>‹</button>
        <span style={{ fontSize:15, fontWeight:600, color:'#111', flex:1 }}>
          {labelDia(date)}
        </span>
        {haySeleccion && (
          <button onClick={limpiarSeleccion} style={{ background:'none', border:'none', color:'#5B3DF6', fontSize:14, fontWeight:500, cursor:'pointer' }}>
            Cancelar
          </button>
        )}
      </div>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'0 20px' }}>
        ...
        <p style={{ fontSize:13, fontWeight:600, color:'#5B3DF6', margin:'16px 0 6px' }}>
          Pendientes ({pendingOrdenados.length})
        </p>
        {pendingOrdenados.length === 0 && (
          <p style={{ fontSize:13, color:'#9ca3af', marginBottom:16 }}>Sin tareas pendientes.</p>
        )}
        {pendingOrdenados.map(task => (
          <DayTaskItem
            key={task.id}
            task={task}
            ...
            onDelete={t => setModal({ tipo:'borrar',  task:t })}
        ...
        {completedVisible.length === 0 && (
          <p style={{ fontSize:13, color:'#9ca3af', marginBottom:16 }}>Ninguna completada aún.</p>
        )}
      ...
      {/* ── Barra de selección múltiple ──────────────────────────────
           Aparece SOLO cuando selectedIds.size > 0.
           Desaparece sola cuando vuelve a 0.              */}
      {haySeleccion && (
        <div style={barraSeleccion}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            <button
              onClick={() => setModal({ tipo:'editarVarias' })}
              style={{ padding:'9px 16px', background:'#EDE9FE', color:'#5B3DF6', border:'none', borderRadius:10, fontSize:13, fontWeight:600, cursor:'pointer' }}
            >
              ✏️ Editar
            </button>
            <button
              onClick={() => setModal({ tipo:'borrarVarias' })}
              style={{ padding:'9px 16px', background:'#ef4444', color:'#fff', border:'none', borderRadius:10, fontSize:13, fontWeight:600, cursor:'pointer' }}
            >
              🗑️ Eliminar
            </button>
          </div>
      ...
      {/* Botón + — solo sin selección activa */}
      {!haySeleccion && (
      ...
      {modal?.tipo === 'editar' && (
        <TaskFormNew task={modal.task} defaultDate={date} onClose={() => setModal(null)} />
      )}
      {modal?.tipo === 'borrar' && (
        <ConfirmDialog
          ...
          onConfirm={async () => {
            hideLocally(modal.task.id)   // oculta instantáneamente
            setModal(null)
            await deleteTask(modal.task) // Firestore en segundo plano
          }}
      ...
      {modal?.tipo === 'borrarVarias' && (
        <ConfirmDialog
          ...
          confirmLabel={`Eliminar ${selectedIds.size}`} danger
...
const fab           = { position:'fixed', bottom:'calc(72px + env(safe-area-inset-bottom))', right:20, width:56, height:56, borderRadius:'50%', background:'#5B3DF6', border:'none', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 4px 20px rgba(91,61,246,0.4)', zIndex:100, WebkitTapHighlightColor:'transparent' }
