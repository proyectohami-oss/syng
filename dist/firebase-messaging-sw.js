/**
 * UNIVERSO A — SW push FCM (scope /firebase-cloud-messaging-push-scope)
 * Solo data payload. Un handler push nativo (iOS PWA).
 */
...
self.addEventListener('push', function(event) {
  event.waitUntil((async function() {
    ...
    const d = dataFrom(payload)
    await show(d.title, d.body, d)
  })())
})
  ...
  e.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(function(list) {
      for (const c of list) {
        if ('focus' in c) {
          c.focus()
          if ('navigate' in c) return c.navigate(self.location.origin + path)
          return
        }
      }
