...
export function useRollover(userId) {
  useEffect(() => {
    ...
    const rolloverRef = async (ref) => {
      ...
      snap.docs.forEach(d => {
        ...
        pendientes.forEach(a => itemsParaHoy.push({
          ...a, dia: hoy.getDate(), mes: hoy.getMonth(), anio: hoy.getFullYear()
        }))
    ...
    const correr = async () => {
      const hoy = new Date()
      const hoyKey = `${hoy.getFullYear()}-${hoy.getMonth()}-${hoy.getDate()}`
      const lsKey = `syng_rollover_v3_${userId}_${hoyKey}`
      if (localStorage.getItem(lsKey)) return

      await rolloverRef(collection(db, 'users', userId, 'pizarron'))

      const gruposSnap = await getDocs(collection(db, 'users', userId, 'misGrupos'))
      for (const g of gruposSnap.docs) {
        const data = g.data()
        if (!data.modulo || data.modulo === 'pizarron') {
          await rolloverRef(collection(db, 'grupos', g.id, 'pizarron'))
        }
      }
